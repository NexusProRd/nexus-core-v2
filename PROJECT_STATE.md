...
**Sprint PRE-LAUNCH-08I — mountedRef Bug: isLoading stuck por Strict Mode**
...
**Sprint UX-POLISH-01 — Dashboard UX Polish (w/ DS-01 regalos header)**
- **Badges**: Añadido estado `entregado` a DashboardClient (caía a gris default). Unificado estilo badges a `text-[11px] font-medium px-2 py-0.5 rounded-full border` (era `font-semibold px-2.5 py-1`).
- **EmptyState**: Componente unificado: icon box `w-12 h-12 rounded-2xl` (era `w-14 h-14 rounded-full`), padding default `py-14` (era `py-16`), título `text-sm` fijo (era `text-sm` compact / `text-base` default). Reemplazadas 4 instancias inline en DashboardClient (+2) e InventarioClient (+2) con `<EmptyState>`. PedidosLista mantiene inline por contenido condicional (no productos vs sin resultados) pero alineado a nuevo sizing.
- **DS-01A — Tabs dashboard unificados**: Pill/segment control estandarizado al patrón de vitrina/page.tsx (usado en 6+ lugares). Unificados DashboardClient (chart range: `p-0.5 rounded-lg px-3 py-1 text-xs` → `p-1 rounded-xl px-3 py-2 text-sm font-semibold`) y regalos/page.tsx (Regalos/GiftCards: `-mt-4 underline-style` → pill estándar con gradiente).
- **DS-01B — Header Regalos refinado**: Las tabs de Regalos usaban colores de gradient (`bg-white/15`, `text-white/70`) pero estaban FUERA del header gradient — invisibles sobre fondo blanco. Corregido a colores estándar de dashboard (`bg-slate-100`, `text-slate-500`, active `var(--primary)`). Padding horizontal unificado a `px-4` (match header). Jerarquía: Banner → mt-6 → Tabs → mt-6 → Contenido.
- **Archivos**: EmptyState.tsx, DashboardClient.tsx, InventarioClient.tsx, PedidosLista.tsx, regalos/page.tsx
- **QA**: type-check 0 errors, build PASS.

**Sprint OPS-02 — Centro Operativo V1 Foundation**
- Smart push suppression en SW (clients.matchAll → skip if dashboard visible)
- Gift incoming modal reactivado (eliminado `{false && ...}`)
- Cleanup de estado huérfano tras aprobar regalo (giftPendientes)
- "Gestionar Regalo →" link en modal de regalo entrante
- "Gestionar Pedido →" link en modal de pedidos entrantes
- Eliminado GiftUrlDetector legacy y flujo 'approved' → 'pending' en actions.ts
- Type-check + build: 0 errores

**Sprint OPS-03A — Auditoría Método de Pago y Gestión de Cobro** (auditoría completa, pendiente implementación)

**Sprint OPS-03B — Auditoría Eliminación WhatsApp Automático** (auditoría completa, pendiente implementación)
...
**Regalos V3.6 — R1, D1+D8, gift_config UI, P0s cerrados, UX-GIFT-01A, UX-GIFT-01X, Beta Ready**
...
- Configuración movida de abajo del último modal a entre Reportes y filtros en GiftDashboard
- UX-GIFT-01A: 9 mejoras visuales en canje (gentleFloat, contraste, copiar código, WhatsApp share, spinner, iconos)
- UX-GIFT-01X: contraste fijo en canje/page.tsx — `text-slate-900` → `text-[#0f172a]`, `text-slate-700` → `text-[#334155]` (break dark mode override `html[data-theme="dark"] .text-slate-900 { color: #f1f5f9 }`)
- Build PASS. Typecheck PASS. 5 files modificados, 1 eliminado.
