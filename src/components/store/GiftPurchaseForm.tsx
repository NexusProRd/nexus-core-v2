...
export default function GiftPurchaseForm({ idTienda, whatsappNumber, defaultProduct, autoOpen }: { idTienda: string; whatsappNumber: string; defaultProduct?: Producto; autoOpen?: boolean }) {
  ...
  const [selected, setSelected] = useState<SelectedItem[]>(defaultProduct ? [{ ...defaultProduct, variante_seleccionada: undefined }] : [])
  ...
  const tieneVariantes = (p: Producto) => p.tallas && Array.isArray(p.tallas) && p.tallas.length > 0

  const hayStockVariante = (p: Producto) => {
    if (!tieneVariantes(p)) return true
    return p.tallas!.some(t => t.stock > 0)
  }

  const total = useMemo(() => {
    return selected.reduce((sum, p) => sum + (p.precio_cobrado || p.precio), 0)
  }, [selected])
  ...
  const handleSelectProduct = (p: Producto) => {
    if (tieneVariantes(p)) {
      ...
      setSelected(prev => [...prev, { ...p, variante_seleccionada: undefined }])
      setSearch('')
    }
  }

  const handleVariantConfirm = (talla: string, precioVariant: number | null) => {
    if (!variantPicker) return
    setSelected(prev => [...prev, {
      id: variantPicker.id,
      nombre: `${variantPicker.nombre} (${talla})`,
      precio: variantPicker.precio,
      imagen_url: variantPicker.imagen_url,
      variante_seleccionada: talla,
      precio_cobrado: precioVariant ?? variantPicker.precio,
    }])
  ...
  const removeProduct = (id: string) => {
    setSelected(prev => prev.filter(p => p.id + (p.variante_seleccionada || '') !== id))
  }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const itemsPayload = selected.map(p => ({
        product_id: p.id,
        nombre: p.nombre,
        precio: p.precio_cobrado || p.precio,
    ...
    <>
      ...
      {open && !success && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={e => e.stopPropagation()}>
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-5 sm:p-6 flex flex-col max-h-[90vh] relative">
              ...
              {search.trim() && (
                <div className="mt-1 border border-slate-200 rounded-xl max-h-48 overflow-y-auto divide-y divide-slate-100">
                  {filtered.length === 0 ? (
                    ...
                    filtered.map(p => {
                      const agotado = sinStock(p) && !hayStockVariante(p)
                      const conVariantes = tieneVariantes(p)
                      return (
                      <button key={p.id} onClick={() => agotado ? null : handleSelectProduct(p)} type="button" disabled={agotado}
                        className={`w-full flex items-center gap-3 px-3 py-2.5 text-left transition-colors ${agotado ? 'opacity-50 cursor-not-allowed' : 'hover:bg-[var(--primary)]/5 cursor-pointer'}`}>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-slate-500">
                            {conVariantes ? 'Tallas disponibles' : formatCurrency(p.precio, currencyCode)}
                          </p>
                        </div>
                        {agotado ? (
            ...
            {selected.length > 0 && (
              <div className="flex-shrink-0 mt-3 space-y-1.5">
                ...
                {selected.map((p, idx) => (
                  <div key={p.id + (p.variante_seleccionada || '') + idx}
                    ...
                    <p className="text-xs text-slate-500 flex-shrink-0">{formatCurrency(p.precio_cobrado || p.precio, currencyCode)}</p>
                    <button onClick={() => removeProduct(p.id + (p.variante_seleccionada || ''))} type="button"
            ...
            {variantPicker && (
              <div className="absolute inset-0 bg-white/95 backdrop-blur-sm rounded-2xl z-10 flex flex-col p-5 sm:p-6" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between mb-4">
                  <h4 className="text-sm font-bold text-slate-900">Seleccionar talla</h4>
                  <button onClick={() => setVariantPicker(null)} className="p-1 text-slate-400 hover:text-slate-600">
                    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                </div>
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-12 h-12 rounded-lg bg-slate-100 flex-shrink-0 overflow-hidden">
                    {variantPicker.imagen_url ? (
                      <img src={variantPicker.imagen_url} alt={variantPicker.nombre} className="w-full h-full object-cover" />
                    ) : (
                      <div className="w-full h-full flex items-center justify-center text-slate-300">
                  ...
                  <p className="text-sm font-semibold text-slate-900">{variantPicker.nombre}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {variantPicker.tallas?.map(t => {
                    const agotado = t.stock <= 0
                    return (
                      <button key={t.talla} type="button" disabled={agotado}
                        onClick={() => !agotado && handleVariantConfirm(t.talla, t.precio)}
                        className={`px-4 py-2.5 text-sm font-semibold rounded-xl border transition-all ${
                          agotado
                            ...
                            : 'bg-slate-50 text-slate-700 border-slate-200 hover:border-[var(--primary)] hover:text-[var(--primary)] hover:bg-[var(--primary)]/5 cursor-pointer'
                        }`}>
                        {t.talla}
                      </button>
                    )
                ...
                <p className="text-xs text-slate-400 mt-3">Selecciona la talla deseada para agregar este producto al regalo.</p>
              </div>
            )}
