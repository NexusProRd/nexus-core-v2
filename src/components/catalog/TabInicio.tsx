    ...
    <>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 pt-4 sm:pt-6">
        <div className="flex items-center gap-3">
          <Link href={`/catalogo/${config.idTienda}`} className="flex items-center gap-3 flex-1 min-w-0 hover:opacity-80 transition-opacity">
            {logoUrl && (
              <Image
                src={logoUrl}
                alt={nombreTienda}
                width={40}
                height={40}
                className="rounded-xl ring-1 ring-slate-200 shadow-sm shrink-0"
              />
            )}
            <div className="min-w-0">
              ...
              {mensajeBienvenida && (
                <p className="text-xs text-slate-400 leading-snug truncate max-w-md">
                  {mensajeBienvenida}
                </p>
              )}
            </div>
          ...
          {numeroLimpio && (
            <a
              ...
              className="shrink-0 flex items-center gap-1.5 px-3 py-2 bg-emerald-50 text-emerald-700 rounded-full text-xs font-semibold hover:bg-emerald-100 transition-all border border-emerald-200"
            >
      ...
      {masVendidos.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 mt-6 sm:mt-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg sm:text-xl font-extrabold text-slate-900">Lo más vendido</h2>
            <span className="text-xs text-slate-400 font-medium">🔥 Tendencia</span>
          </div>
      ...
      {/* ===== FOOTER ===== */}
      <footer className="max-w-6xl mx-auto px-4 sm:px-6 mt-8 sm:mt-12 pb-6">
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-slate-100 shadow-sm p-6 sm:p-8">
            ...
            <div>
              ...
              {numeroLimpio ? (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-2 text-xs text-emerald-600 font-semibold hover:text-emerald-700 transition-colors"
                >
              ...
              <div className="flex flex-wrap items-center justify-center gap-2">
                {instagramUrl && (
                  <a href={instagramUrl} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-white rounded-full border border-slate-200 shadow-sm text-[11px] font-semibold text-slate-600 hover:bg-slate-50 hover:shadow transition-all">
