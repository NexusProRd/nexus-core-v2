...
const BANNER_BUILDER_ENABLED = false
...
export default function CatalogoModalPage() {
  ...
  const [tab, setTab] = useState<'whatsapp' | 'catalogo' | 'banner'>('catalogo')
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 space-y-12 sm:space-y-16 sm:py-6 sm:py-10">
      ...
      {/* ======================================================================= */}
      {/* PAGE-LEVEL TABS: WhatsApp | Catálogo                                     */}
      {/* ======================================================================= */}
      <div>
        <div className="flex p-1 rounded-xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700">
          {[
            { key: 'catalogo', icon: '🛍️', label: 'Catálogo', desc: 'Modal emergente para tus clientes' },
            { key: 'whatsapp', icon: '📱', label: 'WhatsApp', desc: 'Story para compartir' },
            ...(BANNER_BUILDER_ENABLED ? [{ key: 'banner', icon: '🎨', label: 'Banner', desc: 'Promociona tu negocio' }] : []),
          ].map(t => (
            <button key={t.key} onClick={() => setTab(t.key as 'whatsapp' | 'catalogo' | 'banner')}
              ...
              <span className="block break-words">{t.icon} {t.label}</span>
              <span className={`text-xs font-normal mt-0.5 block break-words ${
                tab === t.key ? 'text-[var(--primary)]/70 dark:text-slate-400' : 'text-slate-400 dark:text-slate-500'
              }`}>{t.desc}</span>
            </button>
          ))}
        </div>
      ...
      {/* ======================================================================= */}
      {/* WHATSAPP TAB — WhatsApp Story Generator                                 */}
      {/* ======================================================================= */}
      {tab === 'whatsapp' && (() => {
        ...
        return (
          <div>
            {sectionTitle('Estado para WhatsApp')}
      ...
      {/* ======================================================================= */}
      {/* BANNER TAB — Banner Wizard (disabled: ${BANNER_BUILDER_ENABLED})         */}
      {/* ======================================================================= */}
      {BANNER_BUILDER_ENABLED && tab === 'banner' && (
        <BannerWizard
      ...
      {/* ======================================================================= */}
      {/* UNIFIED HISTORIAL — Catálogo | WhatsApp                                 */}
      {/* ======================================================================= */}
      <div>
        ...
        {/* Historial filter tabs */}
        <div className="flex p-1 rounded-xl bg-slate-100 dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700 mb-4 sm:mb-6">
          {[
            { key: 'catalogo', icon: '🛍️', label: 'Catálogo' },
            { key: 'whatsapp', icon: '📱', label: 'WhatsApp' },
          ].map(f => (
            <button key={f.key} onClick={() => setFiltroHistorial(f.key as 'catalogo' | 'whatsapp')}
              ...
              <div className="text-center py-12 sm:py-16 text-sm text-slate-400 bg-white dark:bg-slate-800/30 rounded-2xl border border-dashed border-slate-200 dark:border-slate-700">
                {cargandoHistorial ? (
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-xs text-slate-300 dark:text-slate-500">Publica una vitrina para que aparezca aquí</p>
                  </div>
                )}
              </div>
            ) : (
