    ...
    <>
      <div className="flex gap-1">
        <button onClick={() => setEditando(true)}
          className="p-2 sm:p-1.5 text-slate-400 hover:text-[var(--primary)] hover:bg-[var(--primary)]/5 rounded-lg transition-colors" title="Editar">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
          </svg>
        </button>
        <button onClick={() => setConfirmDelete(true)}
          className="p-2 sm:p-1.5 text-slate-400 hover:text-rose-600 hover:bg-rose-50 rounded-lg transition-colors" title="Eliminar">
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {editando && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
            <h3 className="text-lg font-bold text-slate-900 mb-4">Editar Producto</h3>
            <form action={actualizarProducto} className="space-y-4">
              ...
              <div>
                ...
                <textarea name="descripcion" rows={3} defaultValue={producto.descripcion || ''}
                  className="w-full px-3 py-2 text-slate-900 border border-slate-200 rounded-xl text-sm focus:ring-2 focus:ring-[var(--primary)] outline-none resize-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Código de Barra / SKU</label>
              ...
              <label className="flex items-center gap-2 cursor-pointer">
                ...
                <span className="text-sm font-medium text-slate-700">Precio de Oferta</span>
              </label>
              {enOfertaEdit && (
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Precio Rebajado</label>
                  <input type="number" name="precio_oferta" step="0.01"
                    value={precioOfertaEdit} onChange={e => setPrecioOfertaEdit(e.target.value)}
                    className="w-full px-3 py-2 text-slate-900 border border-rose-200 rounded-xl text-sm focus:ring-2 focus:ring-rose-500 outline-none" />
              ...
              {(tipoArticulo || usaVariantes) && (
                <>
                  <div className="flex items-center justify-between py-1">
                    <label className="text-xs font-medium text-slate-700">¿Múltiples tallas o variantes?</label>
                    <button type="button" role="switch" aria-checked={usaVariantes} onClick={() => setUsaVariantes(!usaVariantes)}
                      className={`relative w-10 h-5 rounded-full transition-colors ${usaVariantes ? 'bg-violet-600' : 'bg-slate-300'}`}>
                  ...
                  {usaVariantes && variantes.length > 0 && (
                    <div className="space-y-1.5">
                      {variantes.map((v, i) => (
                        <div key={i}>
                          {editandoIdx === i ? (
                            <div className="bg-violet-50 rounded-lg p-2 border border-violet-200 text-xs space-y-1.5">
                              <div className="flex items-center gap-1.5">
                                <span className="font-bold text-violet-700 min-w-[32px]">{v.talla}</span>
                                <div className="flex gap-1.5 items-center flex-1">
                                  <label className="text-slate-400">S</label>
                                  <input type="number" value={editForm.stock} onChange={e => setEditForm(f => ({ ...f, stock: e.target.value }))} min={0}
                                    className="w-14 px-1.5 py-1 border border-violet-200 rounded text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-violet-500" />
                                  <label className="text-slate-400">P</label>
                                  <input type="number" value={editForm.precio} onChange={e => setEditForm(f => ({ ...f, precio: e.target.value }))} min={0} step="0.01"
                                    className="w-16 px-1.5 py-1 border border-violet-200 rounded text-xs text-slate-900 focus:outline-none focus:ring-1 focus:ring-violet-500" />
                            ...
                            <div className="flex items-center gap-1.5 bg-slate-50 rounded-lg p-2 border border-slate-200 text-xs cursor-pointer hover:bg-violet-50 hover:border-violet-200 transition-colors"
                              onClick={() => {
                                setEditForm({ stock: String(v.stock), precio: v.precio?.toString() || '', costo: v.costo?.toString() || '', sku: v.sku || '' })
                                setEditandoIdx(i)
                              }}>
                              <span className="font-bold text-violet-700 min-w-[32px]">{v.talla}</span>
                              <span className="text-slate-500">S:<strong className="text-slate-800">{v.stock}</strong></span>
                              {v.precio !== null && <span className="text-slate-500">P:<strong className="text-slate-800">${v.precio}</strong></span>}
                              {v.costo !== null && <span className="text-slate-500">C:<strong className="text-slate-800">${v.costo}</strong></span>}
                              {v.sku && <span className="text-slate-400 max-w-[60px] truncate" title={v.sku}>SKU:{v.sku}</span>}
                  ...
                  {usaVariantes && (
                    <div className="grid grid-cols-12 gap-1.5 items-end">
                      <div className="col-span-3">
                        <label className="block text-[9px] font-semibold text-slate-500 mb-0.5">Talla</label>
                        {customTallaMode ? (
                          <input type="text" value={nuevaVarianteTalla} onChange={e => setNuevaVarianteTalla(e.target.value)} placeholder="Ej: 4XL"
                            className="w-full px-1.5 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-violet-500 text-slate-900" />
                        ) : (
                          <select value={nuevaVarianteTalla} onChange={e => {
                            ...
                            className="w-full px-1.5 py-1.5 border border-slate-200 rounded-lg text-xs focus:outline-none focus:ring-2 focus:ring-violet-500 text-slate-900 bg-white">
      ...
      {confirmDelete && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" onClick={() => setConfirmDelete(false)}>
          <div className="bg-white rounded-2xl shadow-xl max-w-sm w-full p-6 text-center" onClick={e => e.stopPropagation()}>
            ...
            <p className="text-sm text-slate-500 mb-5">{producto.nombre} se eliminará permanentemente.</p>
